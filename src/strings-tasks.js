...
function formatTime(minutes, seconds) {
  ...
  return min.padEnd(5, ':'.concat(sec));
}
...
function countVowels(str) {
  let vowels = 0;
  str.split('').map((a) => {
    if ('aeiouyAEIOUY'.includes(a)) vowels += 1;
    return vowels;
  });
  return vowels;
}
...
function isPalindrome(str) {
  const palindrome = str.toLowerCase().replace(/ /g, '').replace(/[^\w]/g, '');
...
function extractNameFromTemplate(value) {
  return value.match(/\b, .+\b/)[0].slice(2);
}
  ...
  return str
    ...
    .map((a) => {
      ...
      return a === a.toLowerCase()
        ? String.fromCharCode(((a.charCodeAt(0) - 97 + 13) % 26) + 97)
        : String.fromCharCode(((a.charCodeAt(0) - 65 + 13) % 26) + 65);
    })
    .join('');
}
...
function getCardId(value) {
  ...
  return deck.split(',').indexOf(`'${value}'`);
}
